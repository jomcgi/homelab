...
import { Terminal, Search, X, Maximize2, Minimize2, Image, FileCode, Globe, File, ChevronRight } from 'lucide-react';
    ...
    artifacts: [
      {
        ...
        content: '<html><body style="background: #0a0a0a; color: #10b981; padding: 40px; font-family: monospace;"><h1>Webhook Test Page</h1><p>Status: OK</p></body></html>'
      }
    ]
  },
];
...
const SessionManagerUI = () => {
  ...
  const getArtifactIcon = (type) => {
    switch (type) {
      ...
      case 'webpage': return <Globe className="w-3.5 h-3.5" />;
  ...
  return (
    <div className={`flex h-screen bg-zinc-950 text-zinc-100 ${(isDraggingSidebar || isDraggingPreview) ? 'cursor-col-resize select-none' : ''}`}>
      {/* Sidebar */}
      <div 
        ...
        <div className={`${sidebarCollapsed ? 'opacity-0' : 'opacity-100'} transition-opacity duration-200`}>
          ...
          <div className="p-6 border-b border-zinc-800/50">
            ...
            >
              ...
              <kbd className="px-2 py-0.5 bg-zinc-800 rounded text-xs font-mono border border-zinc-700 group-hover:border-zinc-600">⌘K</kbd>
            </button>
          </div>
              ...
              {sessions.map((session, index) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className="flex items-center gap-1">
                      {session.artifacts?.length > 0 && (
                        <div className="w-1 h-1 rounded-full bg-blue-500" title={`${session.artifacts.length} artifacts`} />
                      )}
                      <kbd className="px-1.5 py-0.5 bg-zinc-800/50 rounded text-xs font-mono border border-zinc-700/50 text-zinc-600 ml-1">⌘{index + 1}</kbd>
                    </div>
                  </div>
                  ...
                  <div className="ml-3.5 text-xs text-zinc-600 flex items-center gap-3">
                    ...
                    <span className={isOldSession(session.age_days) ? 'text-amber-600' : ''}>
          ...
          <div className="p-4 border-t border-zinc-800/50">
            ...
            >
              New Session
              <kbd className="px-1.5 py-0.5 bg-zinc-900 rounded text-xs font-mono border border-zinc-700">⌘N</kbd>
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col">
        ...
        {activeSession && (
          <div className="bg-zinc-900/30 border-b border-zinc-800/50 px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-sm font-medium mb-0.5">{activeSession.name}</h2>
                <p className="text-xs text-zinc-600">
                  {activeSession.branch} • {new Date(activeSession.last_active).toLocaleString()}
                </p>
              </div>
            </div>
        ...
        <div id="main-content-area" className="flex-1 flex overflow-hidden">
          ...
          <div 
            ...
            style={{ 
              width: showPreview && hasArtifacts && !previewMaximized ? `${100 - previewWidth}%` : '100%' 
            }}
          >
              ...
              {activeSession ? (
                <div className="h-full">
                  <div className="text-emerald-400 mb-3">
                    <span className="text-zinc-600">~/{activeSession.branch}</span> 
                  ...
                  <div className="space-y-1 text-zinc-500 text-xs">
                    ...
                    <div className="text-emerald-400/70 pl-6">✓ session restored from persistent storage</div>
                    <div className="text-emerald-400/70 pl-6">✓ {activeSession.claude_context.toLowerCase()}</div>
                    {hasArtifacts && (
                      <div className="text-blue-400/70 pl-6">✓ {activeSession.artifacts.length} artifact{activeSession.artifacts.length > 1 ? 's' : ''} available</div>
                    )}
              ...
              <div className="bg-zinc-900/50 border-b border-zinc-800/50 px-4 py-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                  <button
                    ...
                    className="p-1.5 text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800/50 rounded transition-all"
                    title={previewMaximized ? 'Restore (⌘⇧P)' : 'Maximize (⌘⇧P)'}
                  >
                  ...
                  <div className="h-full">
                    ...
                    <div className="border-b border-zinc-800/50 px-4 py-2 flex items-center justify-between bg-zinc-900/30">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <div className="text-sm font-medium text-zinc-300">{selectedArtifact.title}</div>
                          <div className="text-xs text-zinc-600">
                            {getArtifactTypeLabel(selectedArtifact.type)} • {new Date(selectedArtifact.timestamp).toLocaleTimeString()}
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="h-[calc(100%-60px)]">
                      ...
                      ) : selectedArtifact.type === 'html' || selectedArtifact.type === 'react' ? (
                        <iframe
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-start justify-center pt-32 p-4 z-50">
          <div className="bg-zinc-900 rounded-xl border border-zinc-800 w-full max-w-xl shadow-2xl">
            ...
            <div className="flex items-center gap-3 p-4 border-b border-zinc-800">
              ...
              />
              <div className="flex items-center gap-1">
                <kbd className="px-1.5 py-1 bg-zinc-800 rounded text-xs font-mono border border-zinc-700 text-zinc-500">↑↓</kbd>
                <kbd className="px-2 py-1 bg-zinc-800 rounded text-xs font-mono border border-zinc-700 text-zinc-500">ESC</kbd>
              </div>
            </div>

            {/* Quick Actions */}
            <div className="p-2">
              <div className="text-xs text-zinc-600 px-3 py-2 font-medium">Quick Actions</div>
              <button 
                onClick={() => {
                  setShowCreateModal(true);
                  setShowCommandPalette(false);
                ...
                className={`w-full text-left px-3 py-2.5 rounded-lg flex items-center justify-between group transition-colors ${
                  selectedCommandIndex === 0 ? 'bg-zinc-800' : 'hover:bg-zinc-800'
                }`}
              >
                <span className="text-sm text-zinc-300">Create new session</span>
                <kbd className="px-2 py-1 bg-zinc-800 rounded text-xs font-mono border border-zinc-700 text-zinc-600 group-hover:border-zinc-600">⌘N</kbd>
              </button>
              <button 
                onMouseEnter={() => setSelectedCommandIndex(1)}
                className={`w-full text-left px-3 py-2.5 rounded-lg flex items-center justify-between group transition-colors ${
                  selectedCommandIndex === 1 ? 'bg-zinc-800' : 'hover:bg-zinc-800'
                }`}
              >
                <span className="text-sm text-zinc-300">Suspend all sessions</span>
                <kbd className="px-2 py-1 bg-zinc-800 rounded text-xs font-mono border border-zinc-700 text-zinc-600 group-hover:border-zinc-600">⌘⇧P</kbd>
              </button>
            </div>
            ...
            {searchQuery === '' && (
              <div className="p-2 border-t border-zinc-800">
                <div className="text-xs text-zinc-600 px-3 py-2 font-medium">Recent Sessions</div>
                {sessions.slice(0, 5).map((session, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2.5 rounded-lg flex items-center justify-between group transition-colors ${
                        selectedCommandIndex === commandIndex ? 'bg-zinc-800' : 'hover:bg-zinc-800'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="text-sm text-zinc-300">{session.name}</div>
                          <div className="text-xs text-zinc-600 font-mono">{session.branch}</div>
                        </div>
                      </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-6 max-w-md w-full shadow-2xl">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm text-zinc-400 mb-2">Branch (optional)</label>
                <input
                  type="text"
