      ...
      <div className="flex flex-col py-6 max-w-3xl mx-auto w-full box-border">
        {messageGroups.map((group, groupIndex) => (
          <div
            ...
            className="flex flex-col gap-2 px-4 box-border"
          >
        ...
        {isLoading && displayMessages.length === 0 && (
          <div className="flex items-center justify-center p-8">
            <div className="flex gap-1">
              <span
                className="w-1 h-1 bg-blue-500 rounded-full animate-bounce [animation-delay:-0.32s]"
                aria-label="Loading dot 1"
              ></span>
              <span
                className="w-1 h-1 bg-blue-500 rounded-full animate-bounce [animation-delay:-0.16s]"
                aria-label="Loading dot 2"
              ></span>
              <span
                className="w-1 h-1 bg-blue-500 rounded-full animate-bounce"
                aria-label="Loading dot 3"
              ></span>
            </div>
          </div>
        )}
            ...
            return !hasLoadingToolUse ? (
              <div className="flex items-start px-4 mt-2">
                <div className="w-4 h-5 flex-shrink-0 flex items-center justify-center text-foreground relative">
                  <div
                    className="w-2.5 h-2.5 bg-foreground rounded-full mt-3.5 animate-pulse"
