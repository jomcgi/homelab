...
export const Composer = forwardRef<ComposerRef, ComposerProps>(
  function Composer(
    ...
    // Load cached state
    const [cachedState, setCachedState] = useLocalStorage<ComposerCache>(
      "cui-composer",
      {
    ...
    const [selectedPermissionMode, setSelectedPermissionMode] =
      useState<string>(cachedState.selectedPermissionMode);
    ...
    const [localCommands, setLocalCommands] =
      useState<Command[]>(availableCommands);
    ...
    const detectSlashCommandAutocomplete = (
      value: string,
    ...
    const handleAutocompleteSelection = (selection: string) => {
      ...
      if (autocomplete.type === "command") {
        ...
        setTimeout(() => {
          if (textareaRef.current) {
            const newCursorPos =
              autocomplete.triggerIndex + selection.length + 1;
        ...
        setTimeout(() => {
          if (textareaRef.current) {
            const newCursorPos =
              autocomplete.triggerIndex + 1 + selection.length + 1;
            textareaRef.current.setSelectionRange(newCursorPos, newCursorPos);
            textareaRef.current.focus();
            adjustTextareaHeight();
          }
    ...
    const handleTextChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
      ...
      const commandAutocompleteInfo = detectSlashCommandAutocomplete(
        newValue,
        cursorPos,
      );
      if (commandAutocompleteInfo && onFetchCommands) {
        const suggestions = filterCommandSuggestions(
          commandAutocompleteInfo.query,
        );
    ...
    const handleSubmit = (permissionMode: string) => {
      ...
      if (showDirectorySelector && selectedDirectory === "Select directory")
        return;
            ...
            if (autocomplete.suggestions.length > 0) {
              setAutocomplete((prev) => ({
                ...
                focusedIndex:
                  prev.focusedIndex < 0
                    ? 0
                    : (prev.focusedIndex + 1) % prev.suggestions.length,
              }));
            }
            break;
          case "ArrowUp":
          ...
          case "Tab":
            if (!e.metaKey && !e.ctrlKey) {
              ...
              if (autocomplete.suggestions.length > 0) {
                ...
                const targetIndex =
                  autocomplete.focusedIndex >= 0
                    ? autocomplete.focusedIndex
                    : 0;
                    ...
                    >
                      <TooltipProvider>
                        <Tooltip>
                          ...
                          <TooltipContent>
                            <p>
                              {getPermissionModeTitle(selectedPermissionMode)}
                            </p>
                          </TooltipContent>
                        </Tooltip>
                      </TooltipProvider>
                      <DropdownSelector
                        options={[
                          ...
                          },
                          {
                            value: "plan",
                            label: "Plan",
                            description: "Planning mode only",
                          },
                        ]}
