...
const StreamStatusContext = createContext<StreamStatusContextType | undefined>(
  undefined,
);
...
export function StreamStatusProvider({ children }: { children: ReactNode }) {
  ...
  const handleStreamMessage = useCallback(
    (streamingId: string, event: StreamEvent) => {
  ...
  const unsubscribeFromStream = useCallback((streamingId: string) => {
    setSubscribedStreamIds((current) =>
      current.filter((id) => id !== streamingId),
    );
...
export function useStreamStatus() {
  ...
  if (context === undefined) {
    throw new Error(
      "useStreamStatus must be used within a StreamStatusProvider",
    );
  }
  return context;
}
