...
function InspectorApp() {
  ...
  const [sidebarConversationsLimit, setSidebarConversationsLimit] =
    useState("20");
  const [sidebarConversationsOffset, setSidebarConversationsOffset] =
    useState("0");
  const [sidebarConversationsProjectPath, setSidebarConversationsProjectPath] =
    useState("");
  ...
  const registerPush = async () => {
    try {
      ...
      if (permission !== "granted") {
        showJson("notificationsRegister", {
      ...
      if (!status.publicKey) {
        showJson("notificationsRegister", {
          error: "Server missing VAPID public key",
        });
        return;
      }
      // Subscribe
  ...
  const unregisterPush = async () => {
    try {
      ...
      if (!subscription) {
        showJson("notificationsUnregister", {
  ...
  const listConversationsSidebar = async () => {
    try {
      const data = await api.getConversations({
        limit: sidebarConversationsLimit
          ? parseInt(sidebarConversationsLimit)
          : undefined,
        offset: sidebarConversationsOffset
          ? parseInt(sidebarConversationsOffset)
          : undefined,
  ...
  const startConversation = async () => {
    try {
      ...
      if (claudeExecutablePath)
        body.claudeExecutablePath = claudeExecutablePath;
      if (permissionMode && permissionMode !== "default-mode")
        ...
        for (const line of lines) {
          if (line.trim()) {
            ...
            const newLine = (
              ...
              >
                <span className="text-neutral-500">{lineCount}:</span>{" "}
                {jsonLine}
              </div>
            );
  ...
  const listPermissions = async () => {
    try {
      const data = await api.getPermissions({
        ...
        status: (permissionsStatus === "all"
          ? undefined
          : permissionsStatus) as "pending" | "approved" | "denied" | undefined,
      });
  ...
  const renameSession = async () => {
    try {
      ...
      if (renameCustomName.trim() !== "")
        updateData.customName = renameCustomName.trim();
      ...
      if (initialCommitHead.trim() !== "")
        updateData.initialCommitHead = initialCommitHead.trim();
      if (
        sessionPermissionMode.trim() !== "" &&
        sessionPermissionMode !== "keep-current"
      )
  ...
  const makePermissionDecision = async () => {
    try {
      if (!permissionRequestId) {
        showJson("permissionDecisionResult", {
  ...
  const summarizeText = async () => {
    try {
      if (!geminiTextToSummarize) {
        showJson("geminiSummarizeResult", {
    ...
    <div className="flex w-full h-screen min-h-0 font-mono bg-neutral-100 fixed top-0 left-0 right-0 bottom-0">
      ...
      <div className="w-[450px] bg-neutral-200 overflow-y-auto border-r-2 border-neutral-300 p-5 box-border min-h-0">
          ...
          <div className={cn("overflow-hidden", collapsed.status && "hidden")}>
            ...
            <div className="max-h-96 overflow-auto border border-neutral-300 rounded bg-neutral-50 p-2.5 mt-2.5">
              {results.statusResult && (
                <JsonViewer
                  data={results.statusResult}
                  resultId="statusResult"
                />
              )}
            </div>
            {results.statusResult && !results.statusResult.error && (
              <div className="mt-2.5 text-xs text-neutral-600">
                <div>
                  <strong>Claude Version:</strong>{" "}
                  {results.statusResult.claudeVersion}
                </div>
                <div>
                  <strong>Claude Path:</strong>{" "}
                  {results.statusResult.claudePath}
                </div>
                <div>
                  <strong>Config Path:</strong>{" "}
                  {results.statusResult.configPath}
                </div>
                <div>
                  <strong>Active Conversations:</strong>{" "}
                  {results.statusResult.activeConversations}
                </div>
                <div>
          ...
          <div className={cn("overflow-hidden", collapsed.config && "hidden")}>
            ...
            <div className="max-h-96 overflow-auto border border-neutral-300 rounded bg-neutral-50 p-2.5 mt-2.5">
              {results.configGetResult && (
                <JsonViewer
                  data={results.configGetResult}
                  resultId="configGetResult"
                />
              )}
            </div>
            ...
            <div className="mt-4 border-t border-neutral-300 pt-4">
              ...
              <div className="max-h-96 overflow-auto border border-neutral-300 rounded bg-neutral-50 p-2.5 mt-2.5">
                {results.configUpdateResult && (
                  <JsonViewer
                    data={results.configUpdateResult}
                    resultId="configUpdateResult"
                  />
                )}
              </div>
            </div>
        ...
        <div className="bg-white border-t border-b border-neutral-300 p-4 mb-0">
          ...
          <div
            className={cn("overflow-hidden", collapsed.workingDirs && "hidden")}
          >
            <Button
              onClick={getWorkingDirectories}
              className="bg-neutral-800 hover:bg-neutral-700"
            >
            ...
            <div className="max-h-96 overflow-auto border border-neutral-300 rounded bg-neutral-50 p-2.5 mt-2.5">
              {results.workingDirsResult && (
                <JsonViewer
                  data={results.workingDirsResult}
                  resultId="workingDirsResult"
                />
              )}
            </div>
                ...
                {workingDirectories.map((dir: any, index: number) => (
                  <div key={index} className="mb-1">
                    ...
                    <span className="text-[11px] text-neutral-600">
                      ({dir.conversationCount} convs,{" "}
                      {new Date(dir.lastDate).toLocaleDateString()})
                    </span>
                  </div>
                ))}
              </div>
        ...
        <div className="bg-white border-t border-b border-neutral-300 p-4 mb-0">
          ...
          <div
            className={cn("overflow-hidden", collapsed.commands && "hidden")}
          >
            <div className="my-2.5 p-2.5 bg-neutral-50 rounded">
              <Label
                className="font-bold text-neutral-600 text-xs uppercase mb-1"
            ...
            <div className="max-h-96 overflow-auto border border-neutral-300 rounded bg-neutral-50 p-2.5 mt-2.5">
              {results.commandsResult && (
                <JsonViewer
                  data={results.commandsResult}
                  resultId="commandsResult"
                />
              )}
            </div>
          </div>
        ...
        <div className="bg-white border-t border-b border-neutral-300 p-4 mb-0">
          ...
          <div className={cn("overflow-hidden", collapsed.stop && "hidden")}>
            ...
            <div className="max-h-96 overflow-auto border border-neutral-300 rounded bg-neutral-50 p-2.5 mt-2.5">
              {results.stopResult && (
                <JsonViewer data={results.stopResult} resultId="stopResult" />
              )}
            </div>
          </div>
        </div>
              ...
              <div className="flex gap-2.5">
                <div className="flex-1">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setSidebarConversationsLimit(e.target.value)
                    }
                    placeholder="20"
                    className="font-mono"
                  />
                ...
                <div className="flex-1">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setSidebarConversationsOffset(e.target.value)
                    }
                    placeholder="0"
                    className="font-mono"
                  />
                ...
                <div className="flex-1">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setSidebarConversationsProjectPath(e.target.value)
                    }
          ...
          <div className={cn("overflow-hidden", collapsed.rename && "hidden")}>
            <div className="my-2.5 p-2.5 bg-neutral-50 rounded">
              ...
              >
                ...
                {availableSessions.map((session) => {
                  ...
                  if (session.sessionInfo?.continuation_session_id)
                    sessionFlags.push("🔗");
                  if (session.sessionInfo?.initial_commit_head)
                    sessionFlags.push("🔀");
                  if (
                  ...
                  const flagsStr =
                    sessionFlags.length > 0 ? ` ${sessionFlags.join("")}` : "";
                  ...
                  return (
                    ...
                    >
                      ...
                      {displayName.substring(0, 50)}...{flagsStr}
                      {metricsStr} ({date})
                    </option>
                  );
                })}
              ...
              <div className="flex gap-4">
                <div className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      setSessionPinned(checked as boolean)
                    }
                  />
                  <Label
                    htmlFor="sessionPinned"
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      setSessionArchived(checked as boolean)
                    }
                  />
                  <Label
                    htmlFor="sessionArchived"
                    className="text-sm cursor-pointer"
                  >
                    Archived
                  </Label>
            ...
            <div className="my-2.5 p-2.5 bg-neutral-50 rounded">
              ...
              <Select
                ...
                onValueChange={setSessionPermissionMode}
              >
            ...
            <div className="max-h-96 overflow-auto border border-neutral-300 rounded bg-neutral-50 p-2.5 mt-2.5">
              {results.renameResult && (
                <JsonViewer
                  data={results.renameResult}
                  resultId="renameResult"
                />
              )}
            </div>
          </div>
        ...
        <div className="bg-white border-t border-b border-neutral-300 p-4 mb-0">
          ...
          <div
            className={cn("overflow-hidden", collapsed.permissions && "hidden")}
          >
            <div className="my-2.5 p-2.5 bg-neutral-50 rounded">
              <div className="flex gap-2.5">
                <div className="flex-1">
                ...
                <div className="flex-1">
                  ...
                  <Select
                    ...
                    onValueChange={setPermissionsStatus}
                  >
            ...
            <div className="max-h-96 overflow-auto border border-neutral-300 rounded bg-neutral-50 p-2.5 mt-2.5">
              {results.permissionsResult && (
                <JsonViewer
                  data={results.permissionsResult}
                  resultId="permissionsResult"
                />
              )}
            </div>
          </div>
        ...
        <div className="bg-white border-t border-b border-neutral-300 p-4 mb-0">
          ...
          <div
            className={cn(
              "overflow-hidden",
              collapsed.permissionDecision && "hidden",
            )}
          >
            <div className="my-2.5 p-2.5 bg-neutral-50 rounded">
              <Label
        ...
        <div className="bg-white border-t border-b border-neutral-300 p-4 mb-0">
          ...
          <div className={cn("overflow-hidden", collapsed.listDir && "hidden")}>
              ...
              <div className="flex gap-4">
                <div className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      setListRecursive(checked as boolean)
                    }
                  />
                  <Label
                    htmlFor="listRecursive"
                    className="text-sm cursor-pointer"
                  >
                    Recursive
                  </Label>
                ...
                <div className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      setListRespectGitignore(checked as boolean)
                    }
                  />
                  <Label
                    htmlFor="listRespectGitignore"
                    className="text-sm cursor-pointer"
                  >
                    Respect .gitignore
                  </Label>
            ...
            <div className="max-h-96 overflow-auto border border-neutral-300 rounded bg-neutral-50 p-2.5 mt-2.5">
              {results.listResult && (
                <JsonViewer data={results.listResult} resultId="listResult" />
              )}
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border-t border-b border-neutral-300 p-4 mb-0">
          ...
          <div
            className={cn("overflow-hidden", collapsed.readFile && "hidden")}
          >
            <div className="my-2.5 p-2.5 bg-neutral-50 rounded">
              <Label
                className="font-bold text-neutral-600 text-xs uppercase mb-1"
            ...
            <div className="max-h-96 overflow-auto border border-neutral-300 rounded bg-neutral-50 p-2.5 mt-2.5">
              {results.readResult && (
                <JsonViewer data={results.readResult} resultId="readResult" />
              )}
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border-t border-b border-neutral-300 p-4 mb-0">
          ...
          <div
            className={cn(
              "overflow-hidden",
              collapsed.bulkOperations && "hidden",
            )}
          >
            <div className="mb-2.5">
              <h4 className="my-1">Archive All Sessions</h4>
              <p className="text-xs text-neutral-600 my-1">
                Archive all non-archived sessions at once. This action cannot be
                undone.
              </p>
              <Button
              ...
              <div className="max-h-96 overflow-auto border border-neutral-300 rounded bg-neutral-50 p-2.5 mt-2.5">
                {results.archiveAllResult && (
                  <JsonViewer
                    data={results.archiveAllResult}
                    resultId="archiveAllResult"
                  />
                )}
              </div>
            </div>
        ...
        <div className="bg-white border-t border-b border-neutral-300 p-4 mb-0">
          ...
          <div
            className={cn(
              "overflow-hidden",
              collapsed.geminiHealth && "hidden",
            )}
          >
            <Button
              onClick={getGeminiHealth}
              className="bg-neutral-800 hover:bg-neutral-700"
            >
              Check Gemini Health
            </Button>
            <div className="max-h-96 overflow-auto border border-neutral-300 rounded bg-neutral-50 p-2.5 mt-2.5">
              {results.geminiHealthResult && (
                <JsonViewer
                  data={results.geminiHealthResult}
                  resultId="geminiHealthResult"
                />
              )}
            </div>
          </div>
        ...
        <div className="bg-white border-t border-b border-neutral-300 p-4 mb-0">
          ...
          <div
            className={cn(
              "overflow-hidden",
              collapsed.geminiTranscribe && "hidden",
            )}
          >
            <div className="my-2.5 p-2.5 bg-neutral-50 rounded">
              <Label
        ...
        <div className="bg-white border-t border-b border-neutral-300 p-4 mb-0">
          ...
          <div
            className={cn(
              "overflow-hidden",
              collapsed.geminiSummarize && "hidden",
            )}
          >
            <div className="my-2.5 p-2.5 bg-neutral-50 rounded">
              <Label
            ...
            <div className="max-h-96 overflow-auto border border-neutral-300 rounded bg-neutral-50 p-2.5 mt-2.5">
              {results.geminiSummarizeResult && (
                <JsonViewer
                  data={results.geminiSummarizeResult}
                  resultId="geminiSummarizeResult"
                />
              )}
            </div>
          </div>
        ...
        <div className="bg-white border-t border-b border-neutral-300 p-4 mb-0">
          ...
          <div
            className={cn(
              "overflow-hidden",
              collapsed.notifications && "hidden",
            )}
          >
            ...
            <div className="max-h-96 overflow-auto border border-neutral-300 rounded bg-neutral-50 p-2.5 mt-2.5">
              {results.notificationsStatus && (
                <JsonViewer
                  data={results.notificationsStatus}
                  resultId="notificationsStatus"
                />
              )}
              {results.notificationsRegister && (
                <JsonViewer
                  data={results.notificationsRegister}
                  resultId="notificationsRegister"
                />
              )}
              ...
              {results.notificationsTest && (
                <JsonViewer
                  data={results.notificationsTest}
                  resultId="notificationsTest"
                />
              )}
            </div>
          </div>
          ...
          <div className="bg-white border-t border-b border-neutral-300 p-4 mb-0">
              ...
              >
                ...
                {availableSessions.map((session) => {
                  ...
                  return (
                    ...
                    >
                      {session.sessionId.substring(0, 8)}... -{" "}
                      {displayName.substring(0, 50)}...{metricsStr} ({date})
                    </option>
                  );
                })}
              </select>
            ...
            <div className="max-h-96 overflow-auto border border-neutral-300 rounded bg-neutral-50 p-2.5 mt-2.5">
              {results.detailsResult && (
                <JsonViewer
                  data={results.detailsResult}
                  resultId="detailsResult"
                />
              )}
            </div>
          </div>
