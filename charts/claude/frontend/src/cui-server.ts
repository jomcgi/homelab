...
import { createAuthRoutes, setupAuthWebSocket } from "./routes/auth.routes.js";
...
export class CUIServer {
        ...
        if (isDev && ViteExpress) {
          try {
            this.server = this.app.listen(this.port, this.host, () => {
              ...
              // Set up auth terminal WebSocket server
              setupAuthWebSocket(this.server!);
      ...
      const stopResults = await Promise.allSettled(
        activeSessions.map((streamingId) =>
          this.processManager
            ...
            .catch((error) =>
              this.logger.error(
                `Error stopping session ${streamingId}:`,
                error,
              ),
            ),
        ),
  ...
  private setupRoutes(): void {
    ...
    // Auth routes - before auth middleware (Claude login terminal doesn't need API auth)
