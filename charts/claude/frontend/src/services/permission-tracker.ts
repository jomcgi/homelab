...
export class PermissionTracker extends EventEmitter {
  ...
  addPermissionRequest(
    ...
    if (
      ...
      const sessionId = this.conversationStatusManager.getSessionId(
        streamingId || "",
      );
      ...
      if (sessionId) {
        ...
        this.historyReader
          ...
          .catch((error) => {
            logger.error(
              "Failed to fetch conversation metadata for notification",
              error,
            );
            ...
            if (this.notificationService) {
              this.notificationService
                ...
                .catch((err) =>
                  logger.error("Failed to send permission notification", err),
                );
            }
          });
      } else {
        ...
        this.notificationService
          .sendPermissionNotification(request)
          .catch((error) => {
  ...
  getPermissionRequests(filter?: {
    ...
    if (filter?.streamingId) {
      requests = requests.filter(
        (req) => req.streamingId === filter.streamingId,
      );
    }
