...
export class FileSystemService {
  ...
  async listDirectory(
    ...
    this.logger.debug("List directory requested", {
      requestedPath,
      recursive,
      respectGitignore,
    });
    ...
    try {
      ...
      if (errorCode === "ENOENT") {
        ...
        throw new CUIError(
          "ACCESS_DENIED",
          `Access denied to path: ${requestedPath}`,
          403,
        );
      }
  ...
  async readFile(requestedPath: string): Promise<{
    ...
    try {
      ...
      if (!stats.isFile()) {
        throw new CUIError(
          "NOT_A_FILE",
          `Path is not a file: ${requestedPath}`,
          400,
        );
      }
      ...
      if (!this.isValidUtf8(content)) {
        throw new CUIError(
          "BINARY_FILE",
          "File appears to be binary or not valid UTF-8",
          400,
        );
      }
      ...
      if (errorCode === "ENOENT") {
        ...
        throw new CUIError(
          "ACCESS_DENIED",
          `Access denied to file: ${requestedPath}`,
          403,
        );
      }
  ...
  async isGitRepository(dirPath: string): Promise<boolean> {
    try {
      ...
      this.logger.debug("Directory is not a git repository", {
        dirPath,
        error,
      });
      return false;
  ...
  async validateExecutable(executablePath: string): Promise<void> {
    ...
    try {
      ...
      this.logger.error("Error validating executable", error, {
        executablePath,
      });
      throw new CUIError(
