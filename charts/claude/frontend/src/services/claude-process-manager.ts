...
export class ClaudeProcessManager extends EventEmitter {
  ...
  constructor(
    ...
    this.claudeExecutablePath =
      claudeExecutablePath || this.findClaudeExecutable();
  ...
  async startConversation(
    ...
    if (isResume && !workingDirectory && config.resumedSessionId) {
      const fetchedWorkingDirectory =
        await this.historyReader.getConversationWorkingDirectory(
    ...
    return this.executeConversationFlow(
      ...
      isResume && config.resumedSessionId
        ? { resumeSessionId: config.resumedSessionId }
        : {},
      config,
      args,
      spawnConfig,
    ...
    try {
      ...
      if (!process.killed) {
        ...
        const killTimeout = setTimeout(() => {
          if (!process.killed) {
            this.logger.warn(
              "Process not responding to SIGTERM, sending SIGKILL",
              { streamingId, pid: process.pid },
            );
    ...
    return new Promise<SystemInitMessage>((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        if (!isResolved) {
          ...
          let errorMessage =
            "Timeout waiting for system initialization from Claude CLI";
          if (stderrOutput) {
      ...
      const processClosedHandler = ({
        ...
        let errorMessage =
          "Claude CLI process exited before sending system initialization message";
        if (stderrOutput) {
      ...
      const messageHandler = ({
        ...
        if (this.conversationStatusManager && config) {
          ...
          this.statusTracker.registerActiveSession(
            streamingId,
            systemInitMessage.session_id,
          );
          this.logger.debug(
  ...
  private async executeConversationFlow(
    ...
    try {
      ...
      if (this.fileSystemService) {
        await this.fileSystemService.validateExecutable(
          spawnConfig.executablePath,
        );
      }

      this.logger.debug(
        `${operation.charAt(0).toUpperCase() + operation.slice(1)} conversation`,
        {
          streamingId,
          operation,
      ...
      const { NODE_OPTIONS, VSCODE_INSPECTOR_OPTIONS, ...cleanEnv } =
        spawnConfig.env;
      ...
      this.logger.debug("Waiting for spawn validation", {
        streamingId,
        ...loggerContext,
      });
      const delayPromise = new Promise<string>((resolve) => {
        setTimeout(() => {
          this.logger.debug(
            "Spawn validation period passed, process appears stable",
            { streamingId, ...loggerContext },
          );
      ...
      this.logger.debug(
        "Process spawned successfully, waiting for system init message",
        { streamingId, ...loggerContext },
      );
      ...
      if (this.sessionInfoService && this.fileSystemService) {
        try {
          if (await this.fileSystemService.isGitRepository(systemInit.cwd)) {
            const gitHead = await this.fileSystemService.getCurrentGitHead(
              systemInit.cwd,
            );
            if (gitHead) {
          ...
          this.logger.warn(
            "Failed to set initial commit head for new session",
            {
              sessionId: systemInit.session_id,
              cwd: systemInit.cwd,
  ...
  private buildResumeArgs(config: {
    ...
    this.logger.debug("Built Claude resume args", {
      args,
      hasMCPConfig: !!this.mcpConfigPath,
    });
    return args;
  }
  ...
  private buildStartArgs(config: ConversationConfig): string[] {
    this.logger.debug("Building Claude start args", {
      ...
      promptPreview: config.initialPrompt
        ? config.initialPrompt.substring(0, 50) +
          (config.initialPrompt.length > 50 ? "..." : "")
        : null,
      workingDirectory: config.workingDirectory,
      model: config.model,
    });
    ...
    if (this.mcpConfigPath) {
      ...
      if (
        !currentAllowedTools.includes("mcp__cui-permissions__approval_prompt")
      ) {
        args.push("--allowedTools", "mcp__cui-permissions__approval_prompt");
      }
    }

    this.logger.debug("Built Claude args", {
      args,
      hasMCPConfig: !!this.mcpConfigPath,
    });
    return args;
    ...
    try {
      ...
      claudeProcess.on("error", (error: Error & NodeJS.ErrnoException) => {
        this.logger.error("Claude process spawn error", error, {
          ...
          errorSpawnargs: (
            error as Error & NodeJS.ErrnoException & { spawnargs?: string[] }
  ...
  private setupProcessHandlers(
    ...
    this.logger.debug("Setting up process handlers", {
      streamingId,
      pid: process.pid,
    });
    ...
    if (process.stderr) {
      ...
      process.stderr.on("data", (data) => {
        ...
        this.logger.error("Process stderr output received", {
          ...
          containsPermission: stderrContent
            .toLowerCase()
            .includes("permission"),
    ...
    process.on("exit", (code, signal) => {
      this.logger.debug("Process exited", {
        ...
        timestamp: new Date().toISOString(),
        outputBuffer:
          this.outputBuffers.get(streamingId) || "No output captured",
      });
      this.handleProcessClose(streamingId, code);
    });
  }
  ...
  private handleProcessClose(streamingId: string, code: number | null): void {
    ...
    if (this.notificationService && config) {
      ...
      const sessionId =
        this.statusTracker.getSessionId(streamingId) || "unknown";
      ...
      this.historyReader
        ...
        .catch((error: Error) => {
          this.logger.error(
            "Failed to send conversation end notification",
            error,
          );
        });
    }
