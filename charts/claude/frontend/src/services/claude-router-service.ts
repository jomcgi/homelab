...
export class ClaudeRouterService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.Server = (
        module as unknown as { default: RouterServerConstructor }
      ).default;
    } catch (_error) {
      this.logger.warn(
        "@musistudio/llms package not installed. Router service disabled.",
      );
    ...
    try {
      ...
      this.server.addHook(
        "preHandler",
        async (req: HttpRequest, _reply: unknown) => {
          ...
          try {
            ...
            if (this.config.rules && typeof this.config.rules === "object") {
              ...
              // Check for haiku background routing
                ...
                this.config.rules.background
              ) {
                targetModel = this.config.rules.background;
                this.logger.debug(
                  `Routing haiku model ${body.model} -> ${targetModel}`,
                );
              }
              ...
              // Use default rule if available
                ...
                this.logger.debug(
                  `Routing default ${body.model} -> ${targetModel}`,
                );
              }
            }
