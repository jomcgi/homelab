...
const CUI_SERVER_URL =
  process.env.CUI_SERVER_URL ||
  `http://localhost:${process.env.CUI_SERVER_PORT || "3001"}`;
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      logger.error("Failed to notify CUI server", {
    ...
    const notificationData =
      (await response.json()) as PermissionNotificationResponse;
    ...
    while (true) {
      ...
      if (Date.now() - startTime > TIMEOUT) {
        ...
        const timeoutResponse = {
          behavior: "deny",
          message:
            "Permission request timed out after 10 minutes after user did not respond",
        };
        return {
          content: [
            {
      ...
      if (!pollResponse.ok) {
        logger.error("Failed to poll permission status", {
          status: pollResponse.status,
        });
        throw new Error(
          `Failed to poll permission status: ${pollResponse.status}`,
        );
      }

      const { permissions } =
        (await pollResponse.json()) as PermissionsResponse;
      ...
      if (!permission) {
        ...
        if (!allPermissionsResponse.ok) {
          ...
          throw new Error(
            `Failed to fetch all permissions: ${allPermissionsResponse.status}`,
          );
        }

        const { permissions: allPermissions } =
          (await allPermissionsResponse.json()) as PermissionsResponse;
        const processedPermission = allPermissions.find(
          (p) => p.id === permissionRequestId,
        );

        if (processedPermission) {
          if (processedPermission.status === "approved") {
            logger.debug("Permission approved", {
              tool_name,
              permissionRequestId,
            });
...
async function main() {
  ...
  logger.info("MCP Permission server started", {
    cuiServerUrl: CUI_SERVER_URL,
  });
}
