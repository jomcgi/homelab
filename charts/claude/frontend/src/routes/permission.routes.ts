...
export function createPermissionRoutes(
  ...
  router.post("/notify", async (req: RequestWithRequestId, res, next) => {
    ...
    try {
      ...
      const request = permissionTracker.addPermissionRequest(
        toolName,
        toolInput,
        streamingId,
      );
  ...
  router.get("/", async (req: RequestWithRequestId, res, next) => {
    ...
    try {
      ...
      const permissions = permissionTracker.getPermissionRequests({
        streamingId,
        status,
      });
    ...
    async (req: RequestWithRequestId, res, next) => {
      ...
      try {
        ...
        if (
          ...
          !["approve", "deny"].includes(decisionRequest.action)
        ) {
          throw new CUIError(
            "INVALID_ACTION",
