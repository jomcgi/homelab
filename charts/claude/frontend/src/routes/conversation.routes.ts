...
export function createConversationRoutes(
    ...
    async (
      ...
      try {
        ...
        if (req.body.resumedSessionId) {
          try {
            previousMessages = await historyReader.fetchConversation(
        ...
        if (req.body.resumedSessionId) {
          try {
            await sessionInfoService.updateSessionInfo(
              req.body.resumedSessionId,
              {
            ...
            logger.warn(
              "Failed to update original session with continuation ID",
              {
                originalSessionId: req.body.resumedSessionId,
                error: error instanceof Error ? error.message : String(error),
              },
          ...
          try {
            ...
            logger.warn(
              "Failed to register resumed session with status manager",
              {
                requestId,
                error: error instanceof Error ? error.message : String(error),
              },
  ...
  router.get(
    ...
    async (
      ...
      try {
        ...
        const conversationsWithStatus = result.conversations.map(
          (conversation) => {
            const status = statusTracker.getConversationStatus(
            ...
            if (status === "ongoing") {
              const streamingId = statusTracker.getStreamingId(
                conversation.sessionId,
              );
              if (streamingId) {
        ...
        const existingSessionIds = new Set(
          conversationsWithStatus.map((c) => c.sessionId),
        );
        const conversationsNotInHistory =
          conversationStatusManager.getConversationsNotInHistory(
            existingSessionIds,
          );
        ...
        const allConversations = [
          ...
          ...conversationsNotInHistory,
        ];

        // Ensure session info entries exist for all conversations
        try {
          await sessionInfoService.syncMissingSessions(
            allConversations.map((c) => c.sessionId),
          );
        } catch (syncError) {
  ...
  router.get("/:sessionId", async (req: RequestWithRequestId, res, next) => {
    ...
    try {
      ...
      try {
        const messages = await historyReader.fetchConversation(
          req.params.sessionId,
        );
        const metadata = await historyReader.getConversationMetadata(
          req.params.sessionId,
        );
        ...
        if (
          historyError instanceof CUIError &&
          historyError.code === "CONVERSATION_NOT_FOUND"
        ) {
          const activeDetails =
            conversationStatusManager.getActiveConversationDetails(sessionId);
    ...
    async (
      ...
      try {
        ...
        if (
          ...
          updates.customName.length > 200
        ) {
          logger.debug("Custom name too long", {
            requestId,
            length: updates.customName.length,
          });
          return res.status(400).json({
            success: false,
            sessionId,
        ...
        if (updates.continuationSessionId !== undefined)
          sessionUpdates.continuation_session_id =
            updates.continuationSessionId;
        ...
        // Update session info
        const updatedFields = await sessionInfoService.updateSessionInfo(
          sessionId,
          sessionUpdates,
        );
