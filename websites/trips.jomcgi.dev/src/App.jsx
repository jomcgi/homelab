...
const routeWaypoints = [
  ...
  { lat: 51.2530, lng: -121.9530, name: 'Lillooet' },
  { lat: 51.8461, lng: -121.2956, name: 'Clinton' },
  { lat: 52.1417, lng: -122.1417, name: '100 Mile House' },
  { lat: 52.9784, lng: -122.4930, name: 'Quesnel' },
  { lat: 53.9171, lng: -122.7497, name: 'Prince George' },
  { lat: 54.3833, lng: -124.2500, name: 'Vanderhoof' },
  { lat: 54.2305, lng: -125.7610, name: 'Burns Lake' },
  { lat: 54.3833, lng: -126.7000, name: 'Houston' },
  { lat: 54.7521, lng: -127.1681, name: 'Smithers' },
  { lat: 55.2500, lng: -127.6000, name: 'New Hazelton' },
  { lat: 55.7500, lng: -128.6000, name: 'Kitwanga' },
  { lat: 56.2500, lng: -129.6000, name: 'Meziadin Junction' },
  { lat: 57.0833, lng: -130.0333, name: 'Bell II' },
  { lat: 57.9167, lng: -130.0333, name: 'Iskut' },
  { lat: 58.4333, lng: -130.0000, name: 'Dease Lake' },
  ...
  { lat: 60.8500, lng: -131.4667, name: 'Teslin' },
  { lat: 60.7212, lng: -135.0568, name: 'Whitehorse' },
  { lat: 61.0500, lng: -137.3833, name: 'Haines Junction' },
...
function generateDenseRoute(waypoints, pointsPerSegment = 50) {
  ...
  for (let i = 0; i < waypoints.length - 1; i++) {
    ...
    for (let j = 0; j < pointsPerSegment; j++) {
      ...
      const nearWildlife = wildlifeSpots.find(w => 
        Math.abs(w.lat - lat) < 0.3 && Math.abs(w.lng - lng) < 0.3
      );
      
      const segmentProgress = (i * pointsPerSegment + j) / ((waypoints.length - 1) * pointsPerSegment);
      const tripDurationMs = 35 * 60 * 60 * 1000;
      const timestamp = new Date(startTime.getTime() + segmentProgress * tripDurationMs);
      ...
      points.push({
        ...
        timestamp,
        location: start.name + (j > pointsPerSegment / 2 ? ` → ${end.name}` : ''),
...
function TripMap({ points, selectedId, onMarkerClick, isLive }) {
  ...
  return (
    <div ref={mapContainer} className="absolute top-0 left-0 right-0 bottom-0 w-full h-full" />
  );
}
...
function ImagePanel({ point, isLive, tripStart, totalFrames, currentIndex, isMobile = false }) {
  ...
  return (
    <div className="h-full flex flex-col bg-zinc-900">
      {/* Header */}
      <div className={`flex-none px-4 py-3 border-b ${isLive ? 'border-red-500/30 bg-red-500/5' : 'border-zinc-800'}`}>
        <div className="flex items-center justify-between">
          <div>
            {isLive && (
              <div className="flex items-center gap-2 mb-1">
                <span className="w-2 h-2 rounded-full bg-red-500 animate-pulse" />
                <span className="text-xs font-medium text-red-400">LIVE VIEW</span>
              </div>
            )}
            <h2 className="font-semibold text-lg">{point.location}</h2>
            <p className="text-sm text-zinc-500">{formatTime(point.timestamp)}</p>
          </div>
          {point.animal && (
      ...
      <div className="flex-1 relative min-h-0 p-4">
        <div className={`w-full h-full rounded-lg overflow-hidden flex items-center justify-center ${
          point.animal ? 'bg-amber-500/5 border border-amber-500/20' : 'bg-zinc-800'
...
export default function App() {
  ...
  return (
    <div className="h-screen w-full bg-zinc-950 text-zinc-100 flex flex-col overflow-hidden">
      ...
      {/* Timeline Controls */}
      <div className={`flex-none border-t bg-zinc-900/90 px-4 py-2 transition-colors ${
