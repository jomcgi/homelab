...
export function DayMap({
  ...
  useEffect(() => {
    ...
    map.current.on('load', () => {
      ...
      const getSunIntensity = (sunPos) => {
        if (!sunPos) return { exaggeration: 0.8, shadowColor: '#cccccc', highlightColor: '#000000' };
      ...
      map.current.addLayer({
        ...
        paint: {
          ...
          'hillshade-illumination-direction': getIlluminationDirection(sunPosition)
        }
      }, firstSymbolLayer?.id);
      ...
      const dedupePoints = (pts) => {
        ...
        for (let i = 1; i < pts.length; i++) {
          ...
          if (dlat > 0.0002 || dlng > 0.0002) { // ~20m
            result.push(curr);
          }
        }
        return result;
      ...
      if (points.length > 0) {
        ...
        if (distance > 0.01) { // Only show end if significantly different from start
  ...
  useEffect(() => {
    ...
    if (map.current.getLayer('hillshade')) {
      map.current.setPaintProperty('hillshade', 'hillshade-illumination-direction', getIlluminationDirection(sunPosition));
      map.current.setPaintProperty('hillshade', 'hillshade-exaggeration', intensity.exaggeration);
      map.current.setPaintProperty('hillshade', 'hillshade-shadow-color', intensity.shadowColor);
      map.current.setPaintProperty('hillshade', 'hillshade-highlight-color', intensity.highlightColor);
    }
    ...
    if (currentPhoto?.lat && currentPhoto?.lng) {
      ...
      if (photoMarkerRef.current) {
        ...
        const animate = (currentTime) => {
          ...
          const lng = startLngLat.lng + (targetLngLat[0] - startLngLat.lng) * eased;
          const lat = startLngLat.lat + (targetLngLat[1] - startLngLat.lat) * eased;
