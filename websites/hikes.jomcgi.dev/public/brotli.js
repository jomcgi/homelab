...
window.BrotliDecompress = async function(arrayBuffer) {
    try {
        ...
        if (!brotli || !brotli.decompress) {
            throw new Error('CRITICAL ERROR: Brotli decompression function not available. The WASM module failed to initialize properly.');
        }
        ...
        console.log(`✅ Successfully decompressed ${compressedData.length} bytes to ${decompressedString.length} characters`);
...
initBrotli().catch(error => {
    console.error('WARNING: Failed to pre-load Brotli WASM module:', error.message);
    console.error('The application will attempt to load it when needed, but may fail.');
});