...
def load_walks_from_db() -> List[Walk]:
    ...
    db_path = Path(__file__).parent.parent.parent / "data" / "walks.db"
...
def main():
    ...
    try:
        ...
        logger.info(f"Bundle sizes - Original: {json_size:,} bytes, Compressed: {compressed_size:,} bytes ({compressed_size/json_size*100:.1f}%)")
        ...
        logger.info(f"Generated bundle with {len(walks)} walks, {walks_with_windows} have viable windows, {total_windows} total windows")
