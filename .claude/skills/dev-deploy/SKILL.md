...
Read the service config from `tools/dev-deploy/services.yaml` (auto-generated — do not edit directly).

This file is generated by `//scripts:generate-dev-deploy-registry` from Chart.yaml annotations. To add a new service, add these annotations to the chart's `Chart.yaml`:

```yaml
annotations:
  dev-deploy.enabled: "true"
  dev-deploy.value-overrides: |
    imagePullSecret.create=false
```

Then run `format` to regenerate the registry.
...
Copy the GHCR pull secret from the service's existing namespace (avoids 1Password operator dependency):

```bash
kubectl get secret ghcr-imagepull-secret -n <service> -o json \
  | jq 'del(.metadata.namespace, .metadata.resourceVersion, .metadata.uid, .metadata.creationTimestamp, .metadata.managedFields, .metadata.annotations, .metadata.ownerReferences, .metadata.labels)' \
  | kubectl apply -n test-<service>-<short-sha> -f -
```

**Important:** The `ownerReferences` and `labels` MUST be stripped. The source secret is owned by a
`OnePasswordItem` CR — if the ownerReference is copied, Kubernetes garbage collection will immediately
delete the secret because the owner doesn't exist in the test namespace.
