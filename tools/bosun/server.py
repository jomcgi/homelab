...
AUTO_APPROVED_TOOLS = [
    ...
    "Bash", "Edit", "Write", "Task", "TaskCreate", "TaskUpdate", "TaskList",
    "SendMessage", "Teammate",  # Enable swarm/team tools
]
                            ...
                            for mi, mblock in enumerate(_extract_mermaid_blocks(text_buf)):
                                ...
                                if self.session_id:
                                    ...
                                    _save_artifact(
                                        ...
                                        str(mi + 1),
                                        {"type": "mermaid", "label": mblock["label"], "data": mblock["code"]},
                                    )
                            streaming_text = False
                            text_buf = ""
                ...
                if isinstance(msg, AssistantMessage):
                    for block in msg.content:
                        if isinstance(block, ToolUseBlock):
                            ...
                            images = _extract_images(content) if isinstance(content, list) else []
                            ...
                            if images:
                                ...
                                if self.session_id:
                                    ...
                                    _save_artifact(
                                        ...
                                        "1",
                                        {"type": "image", "label": block.name or "screenshot",
                                         "data": images[0].get("data", ""),
                                         "mimeType": images[0].get("mimeType", "image/png")},
                                    )
...
async def list_sessions(limit: int = 50, all_projects: bool = True):
    ...
    for f, project_name in all_files:
        ...
        if info:
            ...
            readable_project = project_name.replace("-", "/", 1) if project_name.startswith("-") else project_name
...
async def _stream_tts(client, text: str, summarize: bool, suggest_actions: bool):
    ...
    if summarize or suggest_actions:
        spoken_text, summary_text, actions = await _summarize(client, text, suggest_actions)
    ...
    if len(sentences) <= 1:
        ...
        if pcm:
            wav = _pcm_to_wav(pcm, sample_rate=24000)
            yield json.dumps({"audio": base64.b64encode(wav).decode(), "mime_type": "audio/wav"}) + "\n"
    else:
        # Pipeline: first sentence + rest run concurrently
        first_sentence = sentences[0]
        rest = " ".join(sentences[1:])

        first_task = asyncio.create_task(asyncio.to_thread(_generate_tts_raw, client, first_sentence))
        rest_task = asyncio.create_task(asyncio.to_thread(_generate_tts_raw, client, rest))
...
async def websocket_endpoint(ws: WebSocket):
    ...
    async def drain_queue():
        ...
        while message_queue:
            ...
            _update_status()
            log.info("Draining queued message (%d remaining): %s...", len(message_queue), next_text[:60])
            await ws.send_json({"type": "queue_drain", "text": next_text, "remaining": len(message_queue)})
        ...
        while True:
            ...
            if msg_type == "message":
                ...
                if current_task and not current_task.done():
                    ...
                    _update_status()
                    log.info("Queued message (%d in queue): %s...", len(message_queue), text[:60])
                    await ws.send_json({
                        "type": "queued",
                        "text": text,
                ...
                log.info("Compacting session (directive: %s)", directive[:60] if directive else "none")
                ...
                if summary:
                    ...
                    await ws.send_json({"type": "compacted", "old_session_id": old_sid, "warning": "No summary generated"})
