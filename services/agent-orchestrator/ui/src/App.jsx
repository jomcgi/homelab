        ...
        >
          {activeTag && (
            <div
              ...
              style={{ padding: '8px 12px', cursor: 'pointer', color: '#6b7280', fontSize: 13 }}
            >
              Clear filter
            </div>
          )}
...
function JobRow({ job, selected, dismissed, onSelect }) {
  ...
  const attempt = job.attempts?.length ? job.attempts[job.attempts.length - 1] : null
  return (
    <div
      onClick={onSelect}
      style={{
...
function SubmitModal({ onClose, onSubmit, prefill }) {
  ...
  return (
    ...
    >
      ...
      >
        <h2 style={{ margin: '0 0 20px', fontSize: 18, fontWeight: 600 }}>Submit Job</h2>
...
function HelpOverlay({ onClose }) {
  ...
  return (
    ...
    >
      ...
      >
        <h2 style={{ margin: '0 0 16px', fontSize: 18, fontWeight: 600 }}>Keyboard Shortcuts</h2>
          ...
          <tbody>
            {shortcuts.map(([key, desc]) => (
              <tr key={key}>
                ...
                <td style={{ padding: '6px 0', fontSize: 13, color: '#374151' }}>{desc}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
export default function Dashboard() {
  ...
  useEffect(() => {
    function onMove(e) {
      ...
      setSidebarWidth(Math.max(SIDEBAR_MIN, Math.min(SIDEBAR_MAX, dragInitial.current + delta)))
    }
    function onUp() {
    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', fontFamily: 'system-ui, sans-serif', background: '#f8fafc' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 16, fontSize: 12, color: '#6b7280' }}>
          <span>
            <span style={{ color: '#2563eb', fontWeight: 600 }}>{stats.running}</span> running
          </span>
          <span>
            <span style={{ color: '#6b7280', fontWeight: 600 }}>{stats.pending}</span> pending
          </span>
          <span>
            <span style={{ color: '#16a34a', fontWeight: 600 }}>{stats.succeeded}</span> succeeded
          </span>
          <span>
            <span style={{ color: '#dc2626', fontWeight: 600 }}>{stats.failed}</span> failed
          </span>
        </div>
      ...
      >
        {TABS.map((tab, i) => (
          ...
          >
            {tab.label}
            <span style={{ marginLeft: 4, fontSize: 11, color: '#9ca3af' }}>{i + 1}</span>
          </button>
        ))}
        <div style={{ marginLeft: 'auto' }}>
          <TagDropdown allTags={allTags} activeTag={tagFilter} onSelect={setTagFilter} />
        </div>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        >
          {loading && (
            <div style={{ padding: 32, color: '#9ca3af', textAlign: 'center', fontSize: 13 }}>
              Loading…
            </div>
          )}
          {error && (
          ...
          {!loading && !error && visibleJobs.length === 0 && (
            <div style={{ padding: 40, color: '#9ca3af', textAlign: 'center', fontSize: 13 }}>
              No jobs found.
            </div>
          )}
          {visibleJobs.map((job) => (
